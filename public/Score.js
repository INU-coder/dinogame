...
class Score {
  ...
  constructor(ctx, scaleRatio, stageTable, itemTable, itemController) {
    ...
    this.itemController = itemController; // itemController를 저장
  ...
  update(deltaTime) {
    const currentStageInfo = this.stageTable.find(
  ...
  checkStageChange() {
    for (let i = 0; i < this.stageTable.length; i++) {
      ...
      // 현재 점수가 스테이지 점수 이상이고, 해당 스테지로 변경된 적이 없는 경우
      if (
  ...
  getItem(itemId) {
    const itemInfo = this.itemTable.find((item) => item.id === itemId); // this.itemTabel -> this.itemTable
    if (itemInfo) {
